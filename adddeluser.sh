...
echo "Firstly ensure that you have super user privileges."
...
read -rp "How many users you want to add: " add_num
...
read -rp "How many users you want to delete: " del_num
for (( i = 1; i <= "$del_num"; i++ )); do
  read -p "Enter username #$i you want to remove: " user_del
  deluser "$user_del"
  read -e -i 'y' -rp "Do you want to delete his/her home directory: " del_home
  if [ "$del_home" == 'y' ]; then
    rm -r /home/"$user_del"
  fi
done
